...
public class MiniDinoStateMachine : StateMachine
{
    ...
    [field: SerializeField] public WeaponDamage WeaponLeg{get; private set;} 
    ...
    [field:SerializeField] public bool isDinoEating = false;
    ...
    private void Start()
    {
        ...
        if(isDinoEating)
        {
            SwitchState(new MiniDinoStartEatingState(this));
        }
        else if(PatrolPath != null) 
        { 
            SwitchState(new MiniDinoPatrolPathState(this));
        }
        else{ SwitchState(new MiniDinoIdleState(this));}
    ...
    {
        SwitchState(new MiniDinoImpactState(this));
    }

     private void HandleDie()
    {
        SwitchState(new MiniDinoDeadState(this));
    }
    ...
    public float GetDamageStat(){
        return MiniDinoBaseStats.GetStat(Stat.Damage);
    }
    ...
    public void DesactiveAllMiniDinoWeapon()
    {
        WeaponHead.gameObject.SetActive(false);
        WeaponLeg.gameObject.SetActive(false);
    }
